...
function Root({ children, className, as: Component = "div", ...props }) {
...
function Item({ children, className, as: Component = "div", ...props }) {
  ...
  const isActive = useMemo(
    () => index == context.activeItem,
    [context.activeItem, index]
  );
...
function Content({ children, className, as: Component = "div", ...props }) {
  ...
  useEffect(() => {
    setTargetHeight(
      container.current.children[0].getBoundingClientRect().height
    );
  }, [windowSize]);

  const height = useMemo(
    () => (context.isActive ? targetHeight : 0),
    [context.isActive, targetHeight]
  );
...
export const Details = Object.assign(Root, { Item, Content });
